...
from depth_anything_3.utils.ray_utils import get_pose_from_camray
...
class DA3Feat(nn.Module):
    ...
    def _process_ray_pose_estimation(
        ...
        if "ray" in output and "ray_conf" in output:
            pred_pose, pred_focal_lengths, pred_principal_points = get_pose_from_camray(
                output.ray,
                output.ray_conf,
                output.ray.shape[-3],
                output.ray.shape[-2],
            )
            pred_extrinsic = affine_inverse(pred_pose) # c2w to w2c
